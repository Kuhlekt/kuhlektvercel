...
class ApiDatabase {
  ...
  async addCategory(categories: Category[], name: string, description?: string): Promise<Category[]> {
    console.log("📁 ApiDatabase.addCategory() - Adding new category:", name)
  ...
  async addSubcategory(
    ...
    console.log("📁 ApiDatabase.addSubcategory() - Adding new subcategory:", name)

    const updatedCategories = JSON.parse(JSON.stringify(categories)) // Deep clone
    const parentCategory = updatedCategories.find((cat: Category) => cat.id === parentCategoryId)
    ...
    console.log("✅ Subcategory added successfully:", name)
    return updatedCategories
  }

  // Delete category
  async deleteCategory(categories: Category[], categoryId: string): Promise<Category[]> {
    console.log("🗑️ ApiDatabase.deleteCategory() - Deleting category:", categoryId)
  ...
  async deleteSubcategory(categories: Category[], categoryId: string, subcategoryId: string): Promise<Category[]> {
    console.log("🗑️ ApiDatabase.deleteSubcategory() - Deleting subcategory:", subcategoryId)

    const updatedCategories = JSON.parse(JSON.stringify(categories)) // Deep clone
    const category = updatedCategories.find((cat: Category) => cat.id === categoryId)

    if (!category || !category.subcategories) {
      throw new Error("Category or subcategories not found")
    }

    category.subcategories = category.subcategories.filter((sub: any) => sub.id !== subcategoryId)
